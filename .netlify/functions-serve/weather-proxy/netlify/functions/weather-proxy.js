...
exports.handler = async function(event) {
  ...
  const apiKey = process.env.VITE_KMA_AUTH_KEY;
  ...
  try {
    const response = await axios.get(
      `https://apihub.kma.go.kr/api/typ01/url/kma_sfctm2.php?stn=${stn}&authKey=${apiKey}`,
      // `https://apihub.kma.go.kr/api/typ01/url/kma_sfcdd3.php?stn=${stn}&authKey=${apiKey}`,
      {
        headers: {
    ...
    if (!response.data) {
      ...
      return {
        ...
        body: JSON.stringify({ error: "No data received from weather API" })
      };
    }
    const rawData = response.data;
    const lines = rawData.split("\n");
    const yearPrefix = (/* @__PURE__ */ new Date()).getFullYear().toString().substring(2);
    const dataLine = lines.find((line) => line.includes(yearPrefix));
    if (!dataLine) {
      ...
      return {
        ...
        body: JSON.stringify({ error: "No weather data found in the response" })
      };
    }
    ...
    const weatherData = {
      ...
      ws_avg: parseFloat(columns[2]) === -9 ? 0 : parseFloat(columns[2]),
      wd_max: parseInt(columns[4]) === -9 ? 0 : parseInt(columns[4]),
      ws_max: parseFloat(columns[5]) === -9 ? 0 : parseFloat(columns[5]),
      ws_max_tm: columns[6] === "-9" ? "--:--" : `${columns[6].substring(0, 2)}:${columns[6].substring(2, 4)}`,
      ta_avg: parseFloat(columns[10]) === -99 ? 0 : parseFloat(columns[10]),
      ta_max: parseFloat(columns[11]) === -99 ? 0 : parseFloat(columns[11]),
      ta_min: parseFloat(columns[13]) === -99 ? 0 : parseFloat(columns[13])
    };
    console.log("Weather data retrieved and parsed successfully");
    return {
      statusCode: 200,
      headers: {
        "Access-Control-Allow-Origin": "*",
    ...
    if (error.response) {
      ...
      errorMessage = `API responded with status code: ${statusCode}`;
